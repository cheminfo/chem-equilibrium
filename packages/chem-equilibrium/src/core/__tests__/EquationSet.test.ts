...
import { EquationSet } from '../EquationSet.ts';
...
test('should throw when normalizing an equations set with a circular dependency', () => {
  ...
  // 'E' does not appear in the set, so no equation is rewritten by the solvent
  // elimination, exactly as when the argument was left out
  expect(() => eqSet.getNormalized('E')).toThrow(/circular/);
});
