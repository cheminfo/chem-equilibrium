...
test('getSolutions sweeps the pH and keeps acetate conserved', () => {
  ...
  expect(result.solutions[0]['CH3COO-'] / 1.9952225050461358e-5).toBeCloseTo(
    1,
    9,
  );
  expect(result.solutions[0].CH3CO2H / 0.9999800477749495).toBeCloseTo(1, 9);

  // around the pKa (4.7) the two forms are comparable
  expect(result.solutions[5]['CH3COO-'] / 0.6661394245831219).toBeCloseTo(1, 9);
  expect(result.solutions[5].CH3CO2H / 0.333860575416878).toBeCloseTo(1, 9);

  // strongly basic: the conjugated base dominates
  expect(result.solutions[14]['CH3COO-'] / 0.9999999994988128).toBeCloseTo(
    1,
    9,
  );

  for (const solution of result.solutions) {
    expect(solution['CH3COO-'] + solution.CH3CO2H).toBeCloseTo(1, 9);
  }
});
...
test('getTitration returns a decreasing pH curve', () => {
  ...
  for (let i = 1; i < ph.length; i++) {
    expect(ph[i]).toBeLessThan(ph[i - 1]);
  }
});
...
test('getSolutions rejects a missing varying specie', () => {
  ...
  expect(() => serie.getSolutions({ from: 0, to: 1 })).toThrow(
