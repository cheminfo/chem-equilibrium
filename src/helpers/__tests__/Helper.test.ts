...
function getIndexes(labels: string[], model: Model): number[] {
  const indexes = new Array<number>(labels.length);
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    if (i % 2 === 0) {
      if (typeof entry !== 'string') {
        throw new TypeError('even positions must hold a component label');
      }
      componentLabels.push(entry);
    } else {
      if (typeof entry !== 'number') {
        throw new TypeError('odd positions must hold a coefficient');
      }
      coefficients.push(entry);
    }
  }
  const indexes = getIndexes(componentLabels, model);

  const expected = new Array<number>(model.components.length).fill(0);
  for (let i = 0; i < indexes.length; i++) {
    expected[at(indexes, i)] = at(coefficients, i);
  }
  return expected;
}

function getFormedSpecie(
  label: string,
  model: Model,
): ModelFormedSpecies | undefined {
  return model.formedSpecies.find((s) => s.label === label);
}

function getComponent(label: string, model: Model): ModelComponent | undefined {
  return model.components.find((c) => c.label === label);
}

function equationSort(a: EquationJSON, b: EquationJSON): number {
